...
public sealed class UpdateCommentCommandHandlerTests
{
    ...
    private UpdateCommentCommandHandler CreateHandler() =>
    ...
    public async Task Handle_NonMember_Throws404()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(() =>
            CreateHandler().Handle(new UpdateCommentCommand(comment.Id, "Updated"), CancellationToken.None));
    ...
    [Fact]
    public async Task Handle_Author_UpdatesComment()
    {
        var ownerId = Guid.NewGuid();
        var (_, comment) = SetupComment(ownerId);
        _currentUser.Setup(c => c.UserId).Returns(ownerId);

        var result = await CreateHandler().Handle(
            new UpdateCommentCommand(comment.Id, "Updated"),
            CancellationToken.None);

        Assert.Equal("Updated", comment.Body);
        Assert.Equal("Updated", System.Net.WebUtility.HtmlDecode(result.Body));
    ...
    public async Task Handle_NonAuthor_Throws403()
    {
        ...
        await Assert.ThrowsAsync<ForbiddenException>(() =>
            CreateHandler().Handle(new UpdateCommentCommand(comment.Id, "Hijacked"), CancellationToken.None));
