...
public sealed class CardsController(ISender sender) : ControllerBase
{
    ...
    /// <summary>List activity entries for a card. Workspace members only.</summary>
    [HttpGet("api/cards/{id:guid}/activity")]
    ...
    public async Task<IActionResult> GetActivity(Guid id, CancellationToken cancellationToken) =>
        Ok(await sender.Send(new GetCardActivityQuery(id), cancellationToken));
    ...
    /// <summary>Assign a card to a workspace member, or clear the assignee. Requires write access.</summary>
    [HttpPost("api/cards/{id:guid}/assign")]
    [ProducesResponseType(typeof(CardDto), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Assign(
        Guid id,
        [FromBody] AssignCardPayload payload,
        CancellationToken cancellationToken) =>
        Ok(await sender.Send(new AssignCardCommand(id, payload.AssigneeId), cancellationToken));
...
public sealed record AssignCardPayload(Guid? AssigneeId);
